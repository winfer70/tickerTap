...
## Summary
- 3617 nodes · 11681 edges · 160 communities detected
- Extraction: 33% EXTRACTED · 67% INFERRED · 0% AMBIGUOUS · INFERRED: 7865 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## God Nodes (most connected - your core abstractions)
1. `User` - 194 edges
2. `PortfolioPosition` - 130 edges
3. `Notification` - 125 edges
4. `AuditLog` - 121 edges
5. `Portfolio` - 117 edges
...
Cohesion: 0.01
Nodes (274): pop_suppressed_for_digest(), alert_cooldown.py — per (user, ticker, event_type) Telegram-alert cooldown.  M, Returns True if this alert should actually be sent now. If a cooldown     is ac, Returns and clears every cooldown row with suppressed activity for     this use, should_send_alert(), alert_tiers.py — real-time Telegram vs once-a-day digest routing.  Held position, _fetch_map(), cik_ticker_map.py — CIK -> ticker resolution via SEC's free company_tickers.json (+266 more)

### Community 1 - "Community 1"
Cohesion: 0.02
Nodes (307): _apply_stage_result(), _build_soft_stop_report(), _check_condition(), _check_soft_stops(), evaluate_one_soft_stop(), evaluate_price_alerts(), _fetch_daily_close(), _fetch_price() (+299 more)

### Community 2 - "Community 2"
Cohesion: 0.03
Nodes (251): admin_check(), list_audit_logs(), list_users(), lock_account(), lock_user(), admin.py — Admin-only routes for TickerTap.  Provides endpoints for user and a, Lock or unlock a brokerage account atomically.      Eliminates duplication bet, List all registered users, ordered newest-first.      Args:         db: Async (+243 more)

### Community 3 - "Community 3"
Cohesion: 0.03
Nodes (196): create_alert(), delete_alert(), list_alerts(), alerts.py — Price alert CRUD endpoints.  Lets users create, list, update, and, Update an existing price alert.      Args:         alert_id: UUID of the aler, Delete a price alert.      Args:         alert_id: UUID of the alert to delet, Create a new price alert for the current user.      Args:         body: Price, List all price alerts for the current user.      Args:         active_only: I (+188 more)

### Community 4 - "Community 4"
Cohesion: 0.01
Nodes (166): Exception, _call_indicator(), _check_stop_loss(), _compute_indicators(), _eval_comparison(), _eval_condition(), _eval_logical(), _extract_price_series() (+158 more)
...
Cohesion: 0.08
Nodes (97): analyse_stock(), AnalysisResponse, approve_refinement(), ApproveRefinementRequest, _build_prompt(), _call_ollama(), _check_rules(), _compute_atr() (+89 more)

### Community 9 - "Community 9"
Cohesion: 0.04
Nodes (70): BaseHTTPMiddleware, _action_button(), email.py — Email sending utilities for TickerTap.  Provides a generic send_ema, Generate a styled call-to-action button for emails.      Args:         url: T, Send an email via the configured SMTP server.      Args:         to_email: Re, Send a password reset email with a one-time link.      Args:         to_email, Send an email verification link for new account registration.      Args:, Send a verification email to the NEW email address for an email change.      A (+62 more)

### Community 10 - "Community 10"
Cohesion: 0.06
Nodes (59): _assert_account_owner(), Config, HoldingOut, list_holdings(), holdings.py — Holdings (portfolio positions) routes for TickerTap.  Exposes a, Output schema for a single holding position.      Attributes:         holding, Verify the account belongs to the given user. Raises HTTP 403 if not.      Use, List holdings for the given account with pagination (P7.3).      Ownership of (+51 more)

### Community 11 - "Community 11"
Cohesion: 0.04
Nodes (66): _analyze_single_symbol(), _audit(), browse_marketplace(), clone_strategy(), _compute_exit_analysis(), _compute_score(), _compute_trend(), create_composed_strategy() (+58 more)
...
Cohesion: 0.12
Nodes (23): check_webhook_host_safe(), _fire_webhooks(), _insert_notification(), _is_disallowed_ip(), notify(), notify_soft_stop(), trading/notifications.py — Unified notification dispatcher.  Delivers notifica, Send a notification through all enabled channels.      Always creates an in-ap (+15 more)
...
Cohesion: 0.2
Nodes (3): _by_kind(), test_new_position_gets_full_milestone_set(), test_volatile_uses_volatile_bep_trigger()
...
Cohesion: 0.25
Nodes (5): _pred(), _row(), test_format_calls_and_scorecard(), test_prompt_contains_track_record_and_lessons(), test_summarize_track_record()
...
Cohesion: 0.5
Nodes (1): Rules-driven review reminders for open positions (Calendar page).

### Community 128 - "Community 128"
Cohesion: 0.5
Nodes (1): Daily pre-market predictions, post-close grading, and distilled lessons.
...
## Knowledge Gaps
- **835 isolated node(s):** `env.py — Alembic migration environment for TickerTap.  DATABASE_URL environmen`, `Run migrations in 'offline' mode (no live DB connection required).      Genera`, `Run migrations against a live database connection.`, `initial  Revision ID: 0001_initial Revises: Create Date: 2026-01-09 00:00:00`, `password reset tokens  Revision ID: 0002_password_reset_tokens Revises: 0001_` (+830 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 130`** (2 nodes): `dependencies.py`, `dependencies.py — Shared FastAPI dependencies for TickerTap.  Re-exports the c`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 131`** (2 nodes): `templates.py`, `Strategy Templates — pre-built system strategies seeded into the database.  Th`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 132`** (2 nodes): `useContextPopup.js`, `useContextPopup()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 133`** (2 nodes): `useKeyboardShortcuts.js`, `useKeyboardShortcuts()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 134`** (2 nodes): `eslint.config.js`, `globals.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 135`** (1 nodes): `Enforce at least 1 uppercase, 1 lowercase, 1 digit, and 1 special character.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 136`** (1 nodes): `Enforce at least 1 uppercase, 1 lowercase, 1 digit, and 1 special character.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 137`** (1 nodes): `Reject non-positive portfolio values.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 138`** (1 nodes): `Allow only known mode values.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 139`** (1 nodes): `Reject state values outside the allowed set.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 140`** (1 nodes): `Reject schedule values outside the allowed set.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 141`** (1 nodes): `Adjust price for slippage.          Entries slip against you (buy higher, sell`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 142`** (1 nodes): `Build a TradeRecord from an open position and its exit.          Args:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 143`** (1 nodes): `Size a position by risking a fixed percentage of the account.          The num`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 144`** (1 nodes): `Size a position by risking a fixed dollar amount.          Args:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 145`** (1 nodes): `Full Kelly criterion — optimal fraction of account to wager.          Formula:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 146`** (1 nodes): `Fractional Kelly — conservative variant.          Multiplies the full Kelly fr`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 147`** (1 nodes): `ATR-based position sizing.          Uses Average True Range to set a volatilit`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 148`** (1 nodes): `Fetch the latest quote for *symbol*.          Args:             symbol: Ticke`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 149`** (1 nodes): `Return the list of bar intervals this provider supports.          Returns:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 150`** (1 nodes): `Return the maximum historical lookback for *interval*.          Args:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 151`** (1 nodes): `Remove duplicate timestamps and sort chronologically.          Args:`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 152`** (1 nodes): `Check if NYSE is in regular trading hours.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 153`** (1 nodes): `Return a numeric sort key so intervals order from shortest to longest.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 154`** (1 nodes): `The key regression this test guards: before the watchlist join,         a user`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 155`** (1 nodes): `Watchlist tickers have no shares/cost basis — they must not be         folded i`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 156`** (1 nodes): `setup.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 157`** (1 nodes): `chartStyles.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 158`** (1 nodes): `shared.js`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `User` connect `Community 2` to `Community 0`, `Community 3`, `Community 5`, `Community 9`, `Community 18`, `Community 23`?**
  _High betweenness centrality (0.049) - this node is a cross-community bridge._
- **Why does `RuleContext` connect `Community 6` to `Community 3`, `Community 4`?**
  _High betweenness centrality (0.034) - this node is a cross-community bridge._
- **Why does `RuleAlertData` connect `Community 6` to `Community 3`, `Community 4`?**
  _High betweenness centrality (0.034) - this node is a cross-community bridge._
...
- **Are the 122 inferred relationships involving `Notification` (e.g. with `routes/trading.py — Trading AI API endpoints.  Provides strategy CRUD, backtes` and `Validate the X-Internal-Key header and optional IP restriction.      Uses the`) actually correct?**
  _`Notification` has 122 INFERRED edges - model-reasoned connections that need verification._
